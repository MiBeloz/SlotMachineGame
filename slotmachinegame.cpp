...
SlotMachineGame::SlotMachineGame(const sf::VideoMode& videoMode, const sf::Font& font, const sf::Texture& backgroundTexture, std::vector<Symbol>& symbols) {
    ...
    m_buttons.push_back(std::make_unique<Button>("Start", font, 24, sf::Vector2f(50, 440), sf::Vector2f(150, 50)));
    m_buttons.push_back(std::make_unique<Button>("Stop", font, 24, sf::Vector2f(50, 500), sf::Vector2f(150, 50)));

    for (int i = 0; i < REELS_COUNT; ++i) {
        m_reels.push_back(std::make_unique<Reel>(symbols, sf::Vector2f(75 + i * 105, 10)));
    }

    m_labels.emplace_back(std::make_unique<Label>("", font, 100, sf::Vector2f(350, 350)));
    m_labels.emplace_back(std::make_unique<Label>("", font, 30, sf::Vector2f(350, 500)));
